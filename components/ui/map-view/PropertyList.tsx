...
import { List, ListItem, ListItemIcon, ListItemText, IconButton, ListItemAvatar, Avatar, Typography, Divider } from '@mui/material';
...
const PropertyList = ({showList, setShowList, setPositionLeft, properties, changeSelectedProperty}: IProps) => {
  return (
    <div className="property-list" style={{
      ...
      <List sx={{ width: '100%', maxWidth: 360, height: '100%', overflowY: 'auto' }}>
        {
          properties.map((property: PropertyData) => (
            <div key={property.ListingId}>
              <ListItem alignItems="flex-start" style={{cursor: 'pointer'}} onClick={() => changeSelectedProperty(property)}>
                <ListItemAvatar>
                  <Avatar alt="avatar" src={property.Media?.length ? property.Media[0]?.MediaURL : '?'} />
                </ListItemAvatar>
                <ListItemText
